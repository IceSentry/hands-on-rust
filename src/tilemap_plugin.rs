...
use crate::tilemap_plugin::render::TileRenderData;
...
fn setup(
    ...
    for layer_builder_data in &tilemap_builder.layers {
        ...
        build_layer(
            layer_builder_data.id,
            layer_settings,
            material_handle.clone(),
        );
        build_layer(layer_builder_data.id + 1, layer_settings, material_handle);
    }
    ...
    for layer_builder_data in &tilemap_builder.layers {
        ...
        let entity = commands.spawn().insert(layer_builder_data.build()).id();
